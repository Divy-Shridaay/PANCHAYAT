...
import {
  deleteAllLocalFundRevenue,
  deleteLocalFundRevenue,
  fetchLocalFundRevenuePage,
...
export default function LocalFundRevenue() {
  ...
  const [localFundRevenues, setLocalFundRevenues] = useState({});
  ...
  const fetchLocalFundRevenues = async () => {
    const response = await fetchLocalFundRevenuePage(
      page,
      limit,
      searchQuery,
      "villager",
    ...
    setLocalFundRevenues(response.data);
  };

  useEffect(() => {
    setTimeout(() => {
      fetchLocalFundRevenues();
    }, 0);
  }, []);
  ...
  const handleLocalVasulat = async (receiptData) => {
    try {
  ...
  const handlePrint = async () => {
    ...
    if (response) {
      ...
      handleLocalVasulat(recieptData);
    }
  };
    ...
    <Center w={"90%"}>
      <VStack spacing={0} w="100%" align="stretch">
        <HStack justifyContent="space-between" w="100%" p={0}>
          <PageHeader>
            {t("topbar.localFund")} {t("topbar.collection")}
          </PageHeader>
          {user?.role.permissions.includes("LOCAL_FUND_REVENUE_DELETE") && (
            <CustomButton
              colorScheme="red"
              variant={"outline"}
              onClick={() => {
        ...
        <TableComponent
          caption={t("revenue.localCaption")}
          columns={columns}
          data={localFundRevenues?.data || []}
          emptyMessage={t("common.emptyMessage")}
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          onOpen={onOpen}
          totalDocs={localFundRevenues?.pagination?.totalDocuments}
          lastPage={localFundRevenues?.pagination?.lastPage}
          page={page}
          setPage={setPage}
          limit={limit}
          setLimit={setLimit}
          ...
          setId={setId}
          deleteFunction={(id) => {
            deleteLocalFundRevenue(id);
            fetchLocalFundRevenues();
          }}
          handlePrint={handlePrint}
          // isUpdate={false}
          // isDelete={false}
          isCreate={user?.role.permissions.includes("LOCAL_FUND_REVENUE_CREATE")}
          isUpdate={user?.role.permissions.includes("LOCAL_FUND_REVENUE_UPDATE")}
          isDelete={user?.role.permissions.includes("LOCAL_FUND_REVENUE_DELETE")}
        />
        {isOpen && (
                ...
                <AlertDialogFooter>
                  ...
                  <CustomButton
                    ...
                    onClick={async () => {
                      try {
                        onDeleteAllClose();
                        await deleteAllLocalFundRevenue(financialYear, village);
