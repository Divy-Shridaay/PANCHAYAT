...
export default function EducationChallan() {
  ...
  const fetchEducationChallan = async () => {
    const response = await fetchChallansPage(
      page,
      limit,
      searchQuery,
      "",
      { ...filter, type: "Education", financialYear, village },
      status
    );
    if (response) {
      setEducationChallans(response.data);
    }
  };
  useEffect(() => {
    setTimeout(() => {
  ...
  const handleLandTenClick = async (id) => {
    try {
      ...
      const landReportData = await getEducationCessReport(
        1,
        1000000,
        village,
        financialYear
      ...
      let allEducationReportData = landReportData?.data?.data || [];
      let totalReport = allEducationReportData.find(
      ...
      const response = await fetch("/reports/educationGamnoNamunoNo10.html");
  ...
  return (
    <Center w={"90%"}>
      <VStack spacing={0} w="100%" align="stretch">
        <PageHeader>
          {t("topbar.educationCess")} {t("topbar.challan")}
        </PageHeader>

        <TableComponent
          caption={t("challan.caption")}
          columns={columns}
          data={educationChallans?.data || []}
          emptyMessage={t("common.emptyMessage")}
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          onOpen={onOpen}
          totalDocs={educationChallans?.pagination?.totalDocuments}
          lastPage={educationChallans?.pagination?.lastPage}
          page={page}
          setPage={setPage}
          limit={limit}
          setLimit={setLimit}
