...
import LocalFundReportTable from "../components/LocalFundReportTable";
...
export default function LandReports() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await getLocalFundReport(page, limit, village, financialYear);
      if (response?.data) {
  ...
  return (
    <Center w={"90%"}>
      <VStack w={"100%"} spacing={4} align="stretch" mt={3}>
        {/* <HStack w={["100%", "50%"]}> */}
            <PageHeader>{t("topbar.localFund")} {t("topbar.report")}</PageHeader>
        <FormControl w={"200px"}>
