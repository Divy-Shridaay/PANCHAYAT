...
import {
  deleteAllEducationRevenue,
  deleteEducationCess,
  fetchEducationCessPage,
...
export default function EducationRevenue() {
  const { t } = useTranslation();
  const { user } = useUser();
  const { financialYear } = useFinancialYear();
  const { village, talukaName, districtName, villageName } = useVillage();
  ...
  const [educationRevenues, setEducationRevenues] = useState({});
  ...
  const fetchEducationRevenues = async () => {
    const response = await fetchEducationCessPage(
      page,
      limit,
      searchQuery,
      "villager",
    ...
    setEducationRevenues(response.data);
  };

  useEffect(() => {
    setTimeout(() => {
      fetchEducationRevenues();
    }, 0);
  }, []);
  ...
  const handleEducationVasulat = async (receiptData) => {
    try {
  ...
  const handlePrint = async () => {
    ...
    if (response) {
      ...
      handleEducationVasulat(recieptData);
    }
  };
    ...
    <Center w={"90%"}>
      <VStack spacing={0} w="100%" align="stretch">
        <HStack justifyContent="space-between" alignItems="center" p={0}>
          <PageHeader>
            {t("topbar.educationCess")} {t("topbar.collection")}
          </PageHeader>

          {user?.role.permissions.includes("EDUCATION_REVENUE_DELETE") && (
            <CustomButton
              colorScheme="red"
              variant={"outline"}
              onClick={() => {
        ...
        <TableComponent
          caption={t("revenue.educationCaption")}
          columns={columns}
          data={educationRevenues?.data || []}
          emptyMessage={t("common.emptyMessage")}
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          onOpen={onOpen}
          totalDocs={educationRevenues?.pagination?.totalDocuments}
          lastPage={educationRevenues?.pagination?.lastPage}
          page={page}
          setPage={setPage}
          limit={limit}
          setLimit={setLimit}
          ...
          isCreate={user?.role.permissions.includes("EDUCATION_REVENUE_CREATE")}
          isUpdate={user?.role.permissions.includes("EDUCATION_REVENUE_UPDATE")}
          isDelete={user?.role.permissions.includes("EDUCATION_REVENUE_DELETE")}
        />
        {isOpen && (
                ...
                <AlertDialogFooter>
                  ...
                  <CustomButton
                    ...
                    onClick={async () => {
                      try {
                        onDeleteAllClose();
                        await deleteAllEducationRevenue(financialYear, village);
