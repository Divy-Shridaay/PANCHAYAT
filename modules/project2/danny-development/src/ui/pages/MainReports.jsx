...
function MainReports() {
    ...
    const handlePreview152 = async (finalReports) => {
        try {
            ...
            // Use template replacement function (more efficient)
            ...
            finalReports.forEach((data) => {
                const filledTemplate = fillTemplate(htmlTemplate, data);
    ...
    const handlePreview148 = async (finalReports) => {
        try {
            ...
            // Efficient template replacement function
            const fillTemplate = (template, data) => {
                return template.replace(/\{\{\s*(\w+)\s*\}\}/g, (match, key) => data[key] ?? "");
            };
            ...
            finalReports.forEach((record) => {
                const htmlBlock = fillTemplate(htmlTemplate, record);
    ...
    const handlePreview154 = async (finalReports) => {
        try {
            ...
            finalReports.forEach((record) => {
                const filledTemplate = fillTemplate(htmlTemplate, record);
                combinedHTML += `<div style="page-break-after: always;">${filledTemplate}</div>`;
            });
    ...
    const handlePrint = async (notice, total) => {
        ...
        if (response && response.data && Array.isArray(response.data.data)) {
            ...
            // Add console logs here to check the response structure
            console.log("Full result top-level keys:", Object.keys(response.data));
            if (response.data.data && response.data.data.length > 0) {
                console.log("sarkari:", response.data.data[0]?.landData?.sarkari);
                console.log("left top level:", response.data.data[0]?.left);
                console.log("collumnTwentyOne:", response.data.data[0]?.landData?.collumnTwentyOne);
            }

            // Optimize data transformation - batch number conversions
const finalReports = resultArray.map((result,index) => {
    ...
    console.log("ALL result keys with values:", JSON.stringify(result, null, 2));
    console.log("landData fields:", result.landData);
    console.log("localFundData fields:", result.localFundData);
    console.log("educationData fields:", result.educationData);
    // left is at TOP LEVEL of result, not inside landData
    const sourceLandPachhliBaki = Number(result.left || 0);  // ✅ 1740.57
    ...
    // FIX 2: educationLeft = 25% of top-level left
    ...
    let data = {
        ...
        // FIX 1: landLeft = top-level left (1740.57)
        landLeft: sourceLandPachhliBaki,
        // FIX 3: landPending = sivay + localFundPending (104 + 227.1 = 331.1)
        landPending: sourceLandSivay + sourceLocalPending,
        landRotating: sourceLandRotating,
        ...
        // FIX 2: educationLeft = 25% of left
        educationLeft: computedEducationLeft,
    ...
    const numericKeys = ['educationPending', 'landTotal', 'localTotal', 'educationTotal', 'totalPending', 'totalLeft', 'total'];
    numericKeys.forEach(key => {
        data[key] = Number(data[key]).toFixed(2);
    });

    data.totalWord = convertToCurrencyWords(data.total);
    data.quarter = (Number(data.total) / 4).toFixed(2);
    data.quarterWord = convertToCurrencyWords(data.quarter);
    data.total2 = (Number(data.total) + Number(data.quarter)).toFixed(2);

    const gujaratiData = {};
    for (const [key, value] of Object.entries(data)) {
        if (key === "date") {
            gujaratiData[key] = convertEngToGujNumber(convertSlashesToDashes(value));
        } else if (typeof value === 'string' && /^\d+(\.\d{2})?$/.test(value)) {
    ...
    return gujaratiData;
            ...
            // Only open preview window after all data is transformed
            if (notice == 152) {
