...
function MainReports() {
  ...
  const processTemplate = (htmlTemplate, record) => {
    ...
    const handlePrint = async (notice, total) => {
        ...
        const noticeFee = 1;
       const isMansa = ["માણસા", "વિજાપુર"].includes(talukaName);
        ...
        if (response && response.data && Array.isArray(response.data.data)) {
            ...
            const finalReports = resultArray.map((result) => {
                ...
                let data = {
                    ...
                    // ✅ Mansa: original logic untouched
                    // ✅ Non-Mansa: use maangnuLeft (actual પાછલી બાકી from Maangnu)
                    landLeft:    isMansa
                        ? sourceLandPachhliBaki
                        : (result.landData?.maangnuLeft || 0),

                    landPending: isMansa
                        ? sourceLandSivay + sourceLocalPending
                        : (result.landData?.sivay || 0),

                    landRotating: isMansa
                        ? sourceLandRotating
                        : (result.landData?.rotating || 0),

                    localLeft:    isMansa
                        ? 0
                        : (result.localFundData?.maangnuLeft || 0),

                    localPending: isMansa
                        ? 0
                        : (result.localFundData?.localFourFivePanding || 0),

                    localRotating: isMansa
                        ? 0
                        : (result.localFundData?.localRotating || 0),

                    educationLeft:    isMansa
                        ? computedEducationLeft
                        : (result.educationData?.maangnuLeft || 0),
                ...
                // STEP 1: Individual fields round
...
// console.log("hideLocalFund:", isMansa ? "true" : "false");
                ...
                data.noticeTotal    = data.noticeFee;

                // STEP 3: Column totals
