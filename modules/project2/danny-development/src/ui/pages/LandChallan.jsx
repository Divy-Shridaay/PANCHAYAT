...
export default function LandChallan() {
  ...
  const [landChallans, setLandChallans] = useState({});
  ...
  const fetchLandChallan = async () => {
    const response = await fetchChallansPage(
      page,
      limit,
      searchQuery,
      "",
      { ...filter, type: "Land", financialYear, village },
      status
    );
    if (response) {
      setLandChallans(response.data);
    }
  };
  useEffect(() => {
    setTimeout(() => {
  ...
  const handleLandTenClick = async (id) => {
    try {
      ...
      let data = {
        ...
        left: Number(totalReport?.left || 0),
        kayam:
  ...
  return (
    <Center w={"90%"}>
      <VStack spacing={0} w="100%" align="stretch">
        <PageHeader>
          {t("topbar.landRevenue")} {t("topbar.challan")}
        </PageHeader>
        <TableComponent
          caption={t("challan.caption")}
          columns={columns}
          data={landChallans?.data || []}
          emptyMessage={t("common.emptyMessage")}
          searchQuery={searchQuery}
          setSearchQuery={setSearchQuery}
          onOpen={onOpen}
          totalDocs={landChallans?.pagination?.totalDocuments}
          lastPage={landChallans?.pagination?.lastPage}
          page={page}
          setPage={setPage}
          limit={limit}
          setLimit={setLimit}
