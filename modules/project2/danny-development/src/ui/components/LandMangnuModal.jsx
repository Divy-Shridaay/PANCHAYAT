...
import { fetchLocalFundMaagnuById, updateLocalFundMaangnu } from "../../adapters/LocalFundMaangnuApiAdapter";
import { p } from "framer-motion/client";
import { fetchEducationMaagnuById, updateEducationMaangnu } from "../../adapters/EducationCessMaangnuAdapter";
...
function LandMangnuModal({ type,
    ...
    const total =
        type === "Local Fund" || type === "Education" ?
    ...
    const generateLocalFundRevenue = async () => {
        const response =
            ...
            //     })
            //     :
            await updateLocalFundMaangnu(id, {
                villager: currentVillager._id,
                left: leftBehind,
                pending,
                sarkari,
    ...
    const generateEducationCess = async () => {
        ...
        //     : 
        const response =
            await updateEducationMaangnu(id, {
                villager: currentVillager._id,
                left: leftBehind,
                pending,
                rotating ,
    ...
    useEffect(() => {
        ...
        const fetchLocalFundMaangnu = async () => {
            const response = await fetchLocalFundMaagnuById(id);
            if (response.data) {
                const villagerResponse = await fetchVillagerById(
                    response?.data?.data?.villager
                );
        ...
        const fetchEducationMaangnu = async () => {
            const response = await fetchEducationMaagnuById(id);
            if (response.data) {
                const villagerResponse = await fetchVillagerById(
                    response?.data?.data?.villager
                );
                        ...
                        {(currentVillager?.accountNo || villager?.accountNo) && (
                            <Table>
                                ...
                                <Tbody>
                                    ...
                                    {/* Pending (Local Fund or Education)
                                    {(type === "Local Fund" || type === "Education") && (
                                        <Tr>
                                    ...
                                    {/* Sarkari (Other than Local Fund or Education) */}
                                    {!(type === "Local Fund" || type === "Education") && (
                                        <Tr>
