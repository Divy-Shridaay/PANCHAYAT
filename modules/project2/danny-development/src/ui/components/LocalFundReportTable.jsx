...
import { fetchFirstMaster } from "../../adapters/MasterApiAdapter";
...
const LocalFundReportTable = ({
  ...
  const { village, villageName, districtName, talukaName } = useVillage();
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await getLocalFundReport(
        1,
        exportLimit,
        village,
        financialYear
  ...
  const handlePrint = async () => {
    ...
    <html>
      <head><title>લોકલ ફંડ રિપોર્ટ</title></head>
  ...
  `);
    ...
    //     const pages = [];
    //     for (let i = 0; i < fullData.length; i += rowsPerPage) {
    //       pages.push(fullData.slice(i, i + rowsPerPage));
    //     }
    ...
    //         const totalDemand = (maangnuLeft + sarkariAmt + sivayAmt + maangnuRotating).toFixed(2);
    //         const totalPaid = (left + pending + fajal + rotating).toFixed(2);

    //         return `
    //       <tr>
    //         <td>${convertEngToGujNumber(item.accountNo)}</td>
    //         <td style="white-space: nowrap; text-align:left;" >${item.name || ""}</td>
    ...
    //         <td>${item.billDate ? convertEngToGujNumber(convertSlashesToDashes(formatToDDMMYYYY(new Date(item.billDate)))) : ""}</td>
    ...
    //         <thead>
    //         <tr>
    //         <th colspan="17" style="font-size:24px;" class="text-center ">
    //           મોજે. ${villageName} ગામ નમુનો નં. ૧૧ લોકલ ફંડ ( ટાળા પત્રક ) ${convertEngToGujNumber(financialYearName)}
    //         </th>
    //         </tr>
    //           <tr>
    //             <th colspan="2"></th>
    ...
    //           </tr>
    //           <tr>
    //             ${[...Array(17)].map((_, i) => `<th>${convertEngToGujNumber(i + 1)}</th>`).join("")}
    //           </tr>
    //         </thead>
    //         <tbody>
    //           ${rowsHtml}
    ...
    //           <span>પેજ નંબર : ${convertEngToGujNumber(pageIndex + 1)} / ${convertEngToGujNumber(pages.length)}</span>
    //         </div>
    //       </div>
    //     </section>
    //   `;
    ...
    //     <head>
    //       <meta charset="UTF-8" />
    //       <title>ગામ નમુનો નં. ૧૧ લોકલ ફંડ ( ટાળા પત્રક )</title>
    //       <style>
    //           body {
    ...
    function wrapGujaratiName(name) {
      ...
      if (words.length === 1) {
        // For long single-word names, wrap every 8 characters
        return name.match(/.{1,8}/g)?.join("<br>") || name;
      } else {
        // For multi-word names, wrap every 4 words
        const parts = [];
    ...
    // --- Utility: Count how many lines a Gujarati name will occupy ---
    function getLineCount(name) {
      if (!name) return 1;
      const words = name.trim().split(/\s+/);
      if (words.length === 1) {
        return Math.ceil(name.length / 8);
      } else {
        return Math.ceil(words.length / 4);
      }
    }

    // --- Pagination based on total line count (not fixed 17 rows) ---
    ...
    for (let i = 0; i < fullData.length; i++) {
      ...
      // if adding this row exceeds page limit → start new page
    ...
    pages.forEach((chunk, pageIndex) => {
      ...
      <tr>
        <td>${convertEngToGujNumber(item.accountNo)}</td>
        <td style="white-space: nowrap; text-align:left;">${wrapGujaratiName(
          item.name
        )}</td>
        <td>${convertEngToGujNumber(maangnuLeft.toFixed(2))}</td>
        <td>${convertEngToGujNumber(sarkariAmt.toFixed(2))}</td>
    ...
    <section class="print-page">
      <table>
        ...
        <tr>
        <th colspan="17" style="font-size:24px;" class="text-center ">
          મોજે. ${villageName} ગામ નમુનો નં. ૧૧ લોકલ ફંડ ( ટાળા પત્રક ) ${convertEngToGujNumber(
        financialYearName
      )}
        </th>
        </tr>
  ...
  `;
    ...
    // inject to print window (same as your code)
    printWindow.document.open();
    printWindow.document.write(`
<html>
<head>
...
`);
    ...
    const kulAkandarManganuPending = sarkariPending + khetiSivayPending;
    ...
    let nivadvasulatPending = kulakandarVasulatPending - kulFajalPending;
...
<head>
  <meta charset="UTF-8">
  <title>ગામનો નમુનો નં. ૧૧ લોકલ ફંડ લાવણી (ટાળા બાકી પત્રક) તારીજ</title>
  <style>
    body {
      font-family: "Noto Sans Gujarati", "Arial Unicode MS", "Noto Sans", sans-serif;
...
<body>
  ...
  <h3 style="text-align: center;">ગામનો નમુનો નં. ૧૧ લોકલ ફંડ લાવણી (ટાળા બાકી પત્રક) તારીજ સને ${convertEngToGujNumber(
    financialYearName
  )}<br>
  ...
  <table style="margin: auto; border: none;">
    <tr style="border: none;">
      ...
      <td style="border: none;">તલાટી ક્મ મંત્રી</td>
...
`);
  ...
  <div class="page">
    ...
    <h4>ગામનો નમૂનો નં.૧૦ લોકલ ફંડ ચલન તારીજ સને ${convertEngToGujNumber(
      financialYearName
    )}</h4>
    <table>
      <thead>
      ...
      <tbody>
        ...
        ${
          ...
          <tr>
            ...
            <td>${convertEngToGujNumber(kulakandarVasulatLeft.toFixed(2))}</td>
            <td>${convertEngToGujNumber(
            ...
            <td>${convertEngToGujNumber(kulakandarVasulatTotal.toFixed(2))}</td>
          </tr>
          <tr>
...
<head>
  <meta charset="UTF-8">
  <title>ગામનો નમૂનો નં.૧૦ લોકલ ફંડ ચલન તારીજ</title>
  <style>
  body {
    font-family: "Noto Sans Gujarati", "Arial Unicode MS", "Noto Sans", sans-serif;
    margin: 0;
...
`);
  ...
  const createExcelFile = (fullData, challanData, remark) => {
    ...
    saveAs(
      blob,
      `LocalFundReport_${villageName || "village"}_${financialYearName || "fy"}.xlsx`
    );
  };
              ...
              {data?.map((item, index) => (
                <Tr key={index}>
                  ...
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.maangnuLeft)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.sarkari)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.sivay)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(
                      parseFloat(item.maangnuRotating || 0)
                    )}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(
                  ...
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.left || 0)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.pending || 0)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.rotating)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(item.fajal)}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {convertEngToGujNumber(
                      (
                  ...
                  <Td border={"1px solid black"} textAlign={"right"}>
                    ...
                    {convertEngToGujNumber(
                      item.collumnFourteen.toFixed(2) || 0
                    )}
                  </Td>
                  <Td border={"1px solid black"} textAlign={"right"}>
                    {/* {(() => {
                    ...
                    {convertEngToGujNumber(item.collumnFifteen.toFixed(2) || 0)}
                  </Td>
...
export default LocalFundReportTable;
